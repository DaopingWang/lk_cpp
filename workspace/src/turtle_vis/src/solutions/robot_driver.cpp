...
int main(int argc, char	**argv)	{
    ...
    while (ros::ok()) {
        ...
        if (col_prevention.driver_status != IDLE && col_prevention.driver_status != EXITGOAL && col_prevention.pg_info_set){
            if(turtlePose_desired_local[0] > col_prevention.odom_x_max_outbound ||
                    turtlePose_desired_local[0] < col_prevention.odom_x_min_outbound ||
                    fabs(turtlePose_desired_local[1] > col_prevention.odom_y_outbound)) {
